import { Disposable, TextDocumentContentProvider, Uri, workspace, commands, ViewColumn, window, ExtensionContext } from "vscode";
...
export class WelcomePageProvider extends Disposable implements TextDocumentContentProvider {
	...
	constructor(context: ExtensionContext, upgraded: boolean) {
		...
		this._disposable = Disposable.from(
			...
			commands.registerCommand(GaugeVSCodeCommands.Welcome, () => {
				return commands.executeCommand(VSCodeCommands.Preview, welcomeUri, ViewColumn.Active, 'Welcome to Gauge').then((success) => {
				}, (reason) => {
	...
	async provideTextDocumentContent(uri: Uri): Promise<string> {
		var rootPath = path.join('out', uri.path);
		const doc = await workspace.openTextDocument(Uri.file(this._context.asAbsolutePath(path.join(rootPath, 'index.html'))));
		var text =  doc.getText();
		var supress = this._context.globalState.get<Boolean>(GAUGE_SUPPRESS_WELCOME);
		return text
