...
suite('Output Channel', () => {
	...
	test('should convert specification path from relative to absolute', (done) => {
		...
		var outputChannel = new OutputChannel(chan, "", "project");
	...
	test('should convert implementation path from relative to absolute', (done) => {
		...
		assert.equal(chan.read(), "      at Object.<anonymous> (" + path.join("project", "tests", "step_implementation.js:24:10)"));
		done();
	});

	test('should not change implementation path from relative to absolute if it is already absolute', (done) => {
		var chan = new MockVSOutputChannel();
		var outputChannel = new OutputChannel(chan, "", "projectRoot");
		outputChannel.appendOutBuf("      at Object.<anonymous> (" + path.join("projectRoot", "tests", "step_implementation.js:24:10") + ")\n");

		assert.equal(chan.read(), "      at Object.<anonymous> (" + path.join("projectRoot", "tests", "step_implementation.js:24:10)"));
		done();
	});
	...
	test('should convert implementation path from relative to absolute for lamda methods', (done) => {
		...
		assert.equal(chan.read(), "      at Object.step (" + path.join("project", "tests", "step_implementation.js:24:10)"));
		done();
	});
	...
	test('should convert implementation path from relative to absolute for lamda methods in hooks', (done) => {
		...
		assert.equal(chan.read(), "      at Object.beforeSpec (" + path.join("project", "tests", "step_implementation.js:24:10)"));
		done();
	});
});